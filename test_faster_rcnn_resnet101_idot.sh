...
do
  CUDA_VISIBLE_DEVICES=1 python3 forward_net.py \
    --path_to_ckpt models/faster_rcnn_resnet101_idot/frozen_inference_graph-149136.pb \
...
do
  ...
  cat ~/IDOT_faster_rcnn_resnet101_train@${THRESHOLD}.txt ~/IDOT_faster_rcnn_resnet101_test@${THRESHOLD}.txt > ~/IDOT_faster_rcnn_resnet101@${THRESHOLD}.txt
done
for SPLIT in train test;
do
